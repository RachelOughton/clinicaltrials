...
```{r, eval=F, echo=T}
install.packages(c("HSAUR", "HSAUR3", "pROC", "survival", "ggfortify"))
```
...
Therefore, to simulate data from a trial like this, we need to specify values for $\mu,\;\tau_M$ and $\sigma^2$. We also need to specify the total number of participants.
...
:::{.exercise #sim-uneqsd}
Adapt your code (or my code) from Exercise \@ref(exr:ex1sim) to take into account the possibility of the outcome variance being different in the two groups. What is the effect on the estimated power if the outcome standard deviation in group $T$ is 25% larger than the standard deviation in group $C$? 
:::
...
```{r sim2n, echo=T, cache=T, fig.cap = "Power estimates for 100 sets of 100 simulations with larger outcome SD"}
sim_vec3 = rep(NA, 100)
for (i in 1:100){
...
```{r, echo=T, cache=T, fig.cap = "Power estimates for 100 sets of 100 simulations with smaller outcome SD"}
sim_vec4 = rep(NA, 100)
for (i in 1:100){
...
```{r, echo=T, cache=T, fig.cap = "Power estimates for 100 sets of 100 simulations with larger outcome SD, with SRS allocation."}
sim_vec5 = rep(NA, 100)
for (i in 1:100){
...
sim_vec6 = rep(NA, 100)
for (i in 1:100){
  ancova_trial_i = ancova_sim_srs(nsim=100, N=155, mu_B=50, mu=60, rho=0.65, tm=2, sd_eps=1, sd_B = 5)
  sim_vec6[i] = ttest_sim_srs(nsim=100, N=155, mu=5, tm=3, sdC=8, sdT=10)
}
...
exp(ci_log_rr)
```
This seems like a relatively narrow CI, but the fact that it comes so close to containing one (the 'null value' for the risk ratio) fits with the very large upper limit on the NNT. Although this treatment appears significant, it may be that it has a very minimal effect in practice.
...
```{r, fig.cap = "Observations and fitted model for combinations of centre (1: top and 2: bottom) for baseline status (poor: left and good: right)"}
