...
The package we need for this section is `survival`, so install and load that now.

The first dataset we will use is `aml`. 
...
The first step is to combine the first two columns into time-to-event data that we can use. We do this using the `Surv` function in the package `survival`, which creates a 'survival object' that we can then use in various other functions. 
...
:::{.exercise}
Use the `Surv` function now on the `aml` data. The output will contain some notation you probably haven't seen before - what does it mean?
:::
...
The next thing we probably want to do is to fit a survival curve to the data. 





This dataset is from a randomised controlled trial of two treatment regimens for lung cancer. 

```{r kmva, fig.cap = "Kaplan Meier curves for the veteran data, split by treatment. Red is for group T, black for group C (placebo)."}
s_va = with(veteran, Surv(time, status))
va_km_fit <- survfit(Surv(time, status)~trt, data=veteran)
plot(va_km_fit, xlab="Time", ylab = "Probability", col=c(1,2))

```
We see that at first the patients in the treatment have a lower survival rate, but around day 200 this changes.