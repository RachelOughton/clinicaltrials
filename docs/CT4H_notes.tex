...
\caption{\label{tab:unnamed-chunk-48}Kaplan-Meier estimator calculations for ovarian cancer dataset.}
\centering
...
\hypertarget{parametric-likelihood-ratio-test}{%
\section{Parametric: likelihood ratio test}\label{parametric-likelihood-ratio-test}}
...
If we take the margins of the table as fixed (by which we mean \(n_j, \,d_j\,n_{Cj}\) and \(n_{Tj}\)), then \(d_{Cj}\) has a \textbf{hypergeometric distribution}.
...
Since the Myeloid dataset is much bigger, we won't go through the rigmorole of making the table, but will instead use an inbuilt R function from the \texttt{survival} package (more on this in practicals).
...
The linear component \(\mathbf{x}_i^T\boldsymbol\beta\) is often called the \textbf{risk score} or \textbf{prognostic index} for participant \(i\), and represents their hazard at time \(t\) relative to the baseline hazard.
