...
\subsection{One-sided or two-sided?}\label{one-sided-or-two-sided}}
...
\caption{\label{fig:starbucks}A rather infamous two-tailed mermaid}
\end{figure}
...
\begin{definition}
The \textbf{minimum detectable effect size} \(\tau_M\) for a particular trial is the smallest value of effect size that is able to be detected with power \(1-\beta\) (for some specified value of \(\beta\)).
\end{definition}
