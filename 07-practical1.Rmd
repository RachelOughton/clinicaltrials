...
There are many, many packages in R that implement methods for designing and analysiing clinical trials (see a list at [CRAN task view](https://cran.r-project.org/web/views/ClinicalTrials.html)). We will look at some of these, and will also write our own code for some tasks.
  
## Allocation

### Some simulated examples {-}

In this practical we will compare different allocation methods using a simulated dataset with the same underlying probability distributions. We consider four factors:
...
In the code block below, the function `participant_gen` simulates a dataset of size `n` from this distribution. The `seed` argument allows us to make reproducible results; for a specific size `n` and seed value `seed` the function will always output exactly the same dataset.
...
We can simulate one dataset from the distribution described, containing 100 participants, as below (leading to the creation of a data frame `test_df`). This dataset is shown in the Table below, and in Figures \@ref(fig:part-gen1a) and \@ref(fig:part-gen1b).
...
::: {.exercise}
Are there any ways in which you think this data set would be vulnerable to leading to a biased trial? Plot the data in various different ways. For example, 
...
will generate bar charts of hypertension for each age group, soloured by gender.
