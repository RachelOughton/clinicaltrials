...
```{r hommel, fig.cap = "Baseline measurements from the Captopril trial @hommel1986effect."}
...
[I DON'T THINK I SHOULD INCLUDE THIS BIT! ONLY IF WE NEED THE ANOVA FUNCTION ELSEWHERE]

```{r, echo=T}
anova(lm_capt) # What is this doing?
```

The estimate of $\hat{\sigma}^2$ is in this table, in the 'Mean sum of squares' column, 'Residuals' row. So we have $\hat{\sigma}^2 = 34.44$. Note that this is the square of the 'Residual standard error' from the linear model summary.
...
```
We see that the $p$-value for the coefficient $\lambda$ (seen in the `arm:baseline` row) is not at all significant (0.97). Therefore we can be confident that there is no need to fit unequal slopes for this dataset.
:::
